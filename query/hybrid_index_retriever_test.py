...
from llama_index.core import VectorStoreIndex, SimpleKeywordTableIndex, KeywordTableIndex
...
class CustomRetriever(BaseRetriever):
    ...
    def _retrieve(self, query_bundle: QueryBundle) -> List[NodeWithScore]:
        ...
        # 这里的keyword因为没有得分所以排序会有问题，默认10条会剔除很多结果，比如只有一个keywords：北京市
...
def get_GPT_table_retriever(nodes):
    # 定义索引
    # KeywordTableIndex会基于promptTemplate用llm来进行分词(如果不设置prompt会使用系统默认的：)
...
def get_hybrid_retriever(nodes):
    # define custom retriever
    vector_retriever = VectorStoreIndex(nodes, show_progress=True).as_retriever()
    keyword_retriever = get_GPT_table_retriever(nodes)
...
if __name__ == '__main__':
    ...
    # 加载索引，定义retriever
    retriever = get_hybrid_retriever(nodes)
